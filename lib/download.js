...
async function getTemplatePath (repo, tmp, template, useCache) {
  const saveDir = await downloadTemplates(repo, tmp, useCache)
...
async function getTemplatesDesc (repo, tmp, useCache) {
  const saveDir = await downloadTemplates(repo, tmp, useCache)
...
function downloadTemplates (repo, tmp, useCache) {
  ...
  return new Promise((resolve, reject) => {
    ...
    download(repo, saveDir, { clone: true }, err => {
      spinner.stop()
      if (err) {
