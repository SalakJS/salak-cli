...
async function generate (name, src, dest, quickBuild, params) {
  const opts = getOptions(name, src)
  const { prompts, helpers } = opts
  const data = params || await ask(prompts, quickBuild)
...
function readFileAndRender (dir, dest, data, destDir) {
  ...
  for (let file of files) {
    // 过滤隐藏文件，如 *.swo, *.swp, .DS_Store
    if (file.match(/sw[op]$|\.DS_Store$/)) {
      continue
    }
